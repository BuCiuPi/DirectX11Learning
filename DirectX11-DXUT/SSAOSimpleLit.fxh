...
VS_OUTPUT VS(VS_INPUT input)
{
	...
	output.SsaoPosH = mul(float4(input.Pos, 1.0f), gWorldViewProjTex);
...
float4 PS(VS_OUTPUT input) : SV_Target
{
	...
	float ambientAccess = gSsaoMap.SampleLevel(samLinear, input.SsaoPosH.xy, 0.0f).r;

    [unroll]
    for (int i = 0; i < 3; ++i)
    {
        ComputeDirectionalLight(gMaterial, gDirLight[i], input.NormalW, toEyeW, outAmbient, outDiffuse, outSpecular);
        ambient += ambientAccess * outAmbient;
        diffuse += outDiffuse;
        spec += outSpecular;
    }
	...
	return float4(ambientAccess.xxx, 1.0f);
	return litColor;
}
...
TessellationApplication::TessellationApplication(HINSTANCE hInstance) : DirectX11Application(hInstance)
{
	mRadius = 20.0f;
...
bool TessellationApplication::Init(int nShowCmd)
{
...
void TessellationApplication::DrawScene()
{
...
void TessellationApplication::UpdateScene(float dt)
{
...
void TessellationApplication::BuildGeometryBuffer()
{
	mVertexCount = 4;

	std::vector<SimpleVertex> vertices(4);
	vertices[0].Pos = XMFLOAT3(-10.0f, 0.0f, +10.0f);
	vertices[1].Pos = XMFLOAT3(+10.0f, 0.0f, +10.0f);
	vertices[2].Pos = XMFLOAT3(-10.0f, 0.0f, -10.0f);
	vertices[3].Pos = XMFLOAT3(+10.0f, 0.0f, -10.0f);

	vertices[0].Tex = XMFLOAT2(1.0f, 0.0f);
...
void TessellationApplication::BuildConstantBuffer()
{
...
void TessellationApplication::CleanupDevice()
{
	ReleaseCOM(mLandVB);
...
void TessellationApplication::BuildFX()
{
	// Compile the vertex shader
	ID3DBlob* pVSBlob = nullptr;
	HRESULT hr = CompileShaderFromFile(L"Tessellation.fxh", "VS", "vs_5_0", &pVSBlob);
	if (FAILED(hr))
	{
		MessageBox(nullptr,
	...
	hr = CompileShaderFromFile(L"Tessellation.fxh", "HS", "hs_5_0", &pHSBlob);
	if (FAILED(hr))
	{
		MessageBox(nullptr,
			L"The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", L"Error", MB_OK);
	...
	hr = CompileShaderFromFile(L"Tessellation.fxh", "DS", "ds_5_0", &pDSBlob);
	if (FAILED(hr))
	{
		MessageBox(nullptr,
			L"The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", L"Error", MB_OK);
	...
	hr = CompileShaderFromFile(L"Tessellation.fxh", "PS", "ps_5_0", &pPSBlob);
	if (FAILED(hr))
	{
		MessageBox(nullptr,
			L"The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", L"Error", MB_OK);
